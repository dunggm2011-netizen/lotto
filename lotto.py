...
# ================================================================
# 1️⃣ CẤU HÌNH - ĐIỀN userId VÀ secretKey CLONE CỦA MÀY
# ================================================================
USER_ID = "11575993"
...
ICONS = {
    ...
    "rocket": "🚀", "shield": "🛡️", "lightning": "⚡", "trophy": "🏆"
}
        ...
        try:
            ...
            if prediction_history:
                ...
                if last_pred.get('actual') is None and current_issue is not None and current_issue >= last_pred['issue']:
                    ...
                    color = "#39ff14" if last_pred['prediction'] == actual else "#ff0055"
...
""", parse_mode='Markdown')
            ...
            if current_issue is not None and current_issue != last_issue:
                ...
                keyboard.add(
                    types.InlineKeyboardButton("✅ ĐÚNG", callback_data=f'result_{next_issue}_ĐÚNG'),
                    types.InlineKeyboardButton("❌ SAI", callback_data=f'result_{next_issue}_SAI'),
                )
...
{pred['reason']}
━━━━━━━━━━━━━━━━━━━
📌 Sau khi có kết quả, bấm nút bên dưới!
...
👑 *LOTTO PREDICT BOT - ADMIN* 👑
━━━━━━━━━━━━━━━━━━━
🤖 ENSEMBLE VIP - 8 thuật toán
👤 Tài khoản: `{USER_ID}`
...
def handle_callback(call):
    ...
    if data.startswith('result_'):
        parts = data.split('_')
        issue = int(parts[1])
        result = parts[2]  # ĐÚNG hoặc SAI
        
        for pred in prediction_history:
            if pred['issue'] == issue and pred.get('actual') is None:
                pred['actual'] = result
                pred['result'] = "✅ ĐÚNG" if result == "ĐÚNG" else "❌ SAI"
...
""", parse_mode='Markdown')
        ...
        bot.answer_callback_query(call.id, "⚠️ Phiên này đã được xác nhận trước đó!")
        return
...
📌 Sau khi có kết quả, bấm nút bên dưới!
"""
        bot.edit_message_text(result_text, chat_id=msg.chat.id, message_id=msg.message_id, parse_mode='Markdown', reply_markup=keyboard)
